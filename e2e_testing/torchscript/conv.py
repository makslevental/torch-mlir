...
#         return torch.ops.aten.convolution(inputVec, weight, bias=None, stride=[1], padding=[0], dilation=[1], transposed=False, output_padding=[0], groups=1)

# @register_test_case(module_factory=lambda: ConvolutionModule1D())
# def ConvolutionModule1D_basic(module, tu: TestUtils):
#     with torch.backends.mkldnn.flags(enabled=False):
#         module.forward(torch.ones(3, 3, 10), torch.ones(3, 3, 2))
...
class ConvolutionModule2D(torch.nn.Module):
    ...
    def forward(self, inputVec, weight):
        return torch.ops.aten.convolution(inputVec, weight, bias=None, stride=[1, 1], padding=[0, 0], dilation=[1, 1], transposed=False, output_padding=[0, 0], groups=1)

@register_test_case(module_factory=lambda: ConvolutionModule2D())
def ConvolutionModule2D_basic(module, tu: TestUtils):
    with torch.backends.mkldnn.flags(enabled=False):
        module.forward(torch.ones(3, 3, 10, 10), torch.ones(3, 3, 2, 2))
...
#     ])
#     def forward(self, inputVec, weight):
#         return torch.ops.aten.convolution(inputVec, weight, bias=None, stride=[1, 1, 1], padding=[0, 0, 0], dilation=[1, 1, 1], transposed=False, output_padding=[0, 0, 0], groups=1)

# @register_test_case(module_factory=lambda: ConvolutionModule3D())
# def ConvolutionModule3D_basic(module, tu: TestUtils):
#     with torch.backends.mkldnn.flags(enabled=False):
#         module.forward(torch.ones(3, 3, 10, 10, 10), torch.ones(3, 3, 2, 2, 2))