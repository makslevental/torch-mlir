#map = affine_map<(d0, d1) -> (d0 + d1)>
module attributes {torch.debug_module_name = "Conv2dNoPaddingOutModule"}  {
  memref.global "private" constant @__constant_10x2x3x3xf32 : memref<10x2x3x3xf32> = dense<"">
  func @forward(%arg0: memref<5x2x10x20xf32>, %arg1: memref<5x10x8x18xf32>) {
    %c5 = arith.constant 5 : index
    %c2 = arith.constant 2 : index
    %c10 = arith.constant 10 : index
    %c3 = arith.constant 3 : index
    %c8 = arith.constant 8 : index
    %c18 = arith.constant 18 : index
    %c0 = arith.constant 0 : index
    %c1 = arith.constant 1 : index
    %0 = memref.get_global @__constant_10x2x3x3xf32 : memref<10x2x3x3xf32>
    scf.for %arg2 = %c0 to %c5 step %c1 {
      scf.for %arg3 = %c0 to %c10 step %c1 {
        scf.for %arg4 = %c0 to %c8 step %c1 {
          scf.for %arg5 = %c0 to %c18 step %c1 {
            scf.for %arg6 = %c0 to %c2 step %c1 {
              scf.for %arg7 = %c0 to %c3 step %c1 {
                scf.for %arg8 = %c0 to %c3 step %c1 {
                  %1 = affine.apply #map(%arg4, %arg7)
                  %2 = affine.apply #map(%arg5, %arg8)
                  %3 = memref.load %arg0[%arg2, %arg6, %1, %2] : memref<5x2x10x20xf32>
                  %4 = memref.load %0[%arg3, %arg6, %arg7, %arg8] : memref<10x2x3x3xf32>
                  %5 = memref.load %arg1[%arg2, %arg3, %arg4, %arg5] : memref<5x10x8x18xf32>
                  %6 = arith.mulf %3, %4 : f32
                  %7 = arith.addf %5, %6 : f32
                  memref.store %7, %arg1[%arg2, %arg3, %arg4, %arg5] : memref<5x10x8x18xf32>
                }
              }
            }
          }
        }
      }
    }
    return
  }
}
