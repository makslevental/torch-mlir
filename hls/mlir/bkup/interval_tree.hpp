...
public:
  node(node *parent, interval_type interval)
      : interval_{std::move(interval)}, max_{interval.high()}, parent_{parent},
        left_{}, right_{}, color_{rb_color::fail} {}
...
public:
  constexpr basic_interval_tree_iterator(basic_interval_tree_iterator const &) =
      default;
  basic_interval_tree_iterator &
  operator=(basic_interval_tree_iterator const &) = default;
...
public:
  ...
  typename value_type::interval_type operator*() const {
    if (node_)
      ...
      assert(
          "dereferencing interval_tree_iterator out of bounds" && false);
  }
  ...
  const_interval_tree_iterator parent() const {
    if (node_)
      ...
      assert("interval_tree_iterator out of bounds" && false);
  }
...
private:
  const_interval_tree_iterator(node_type const *node, tree_type const *owner)
      : basic_interval_tree_iterator<node_type, tree_type const *>{node,
                                                                   owner} {}
};
//############################################################################################################
template <typename node_type>
...
//############################################################################################################
template <typename IntervalT = interval<int, closed>> class interval_tree {
public:
...
public:
  ...
  iterator erase(iterator iter) {
    if (!iter.node_)
      assert("cannot erase end iterator" && false);
...
private:
  ...
  static bool
  find_all_i(typename std::conditional<std::is_same<IteratorT, iterator>::value,
                                       ThisType, ThisType const>::type *self,
             node_type *ptr, interval_type const &ival,
             FunctionT const &on_find, ComparatorFunctionT const &compare) {
