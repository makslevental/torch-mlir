...
template <typename T> std::size_t simple_get_hash(const T &o);
...
auto dispatch_hash(const T &o)
    -> decltype(std::hash<T>()(o), type_if_not_enum<T, std::size_t>()) {
  return std::hash<T>()(o);
}

template <typename T>
typename std::enable_if<std::is_enum<T>::value, std::size_t>::type
dispatch_hash(const T &o) {
...
template <typename... Types> struct hash<std::tuple<Types...>> {
  template <std::size_t idx, typename... Ts> struct tuple_hash {
  ...
  template <typename... Ts> struct tuple_hash<0, Ts...> {
    std::size_t operator()(const std::tuple<Ts...> &t) const {
...
// }
template <typename... Types> std::size_t get_hash(const Types &...args) {
